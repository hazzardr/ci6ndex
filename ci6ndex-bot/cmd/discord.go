...
var startBot = &cobra.Command{
	Use:   "start",
	Short: "start an instance of the discord ci6ndex-bot",
	Long: `
Starts the discord ci6ndex-bot, which will listen for slashCommands and respond to them.
`,
	Run: startBotFunc,
}
...
func startBotFunc(cmd *cobra.Command, args []string) {
	...
	if err != nil {
		slog.Error("Error creating discord ci6ndex-bot", "error", err)
		return
	}
	err = bot.Start()
	if err != nil {
		slog.Error("Error starting discord ci6ndex-bot", "error", err)
		return
	}
}
...
var slashCommands = &cobra.Command{
	Use:   "commands",
	Short: "manage slash (/) commands for the discord ci6ndex-bot",
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) == 0 {
			err := cmd.Help()
			if err != nil {
...
var refreshCmd = &cobra.Command{
	Use:   "refresh",
	Short: "refresh all slash commands from the discord ci6ndex-bot.",
	Long: `
This command will refresh all slash commands registered for the ci6ndex-bot. 
...
func refreshCommandsFunc(cmd *cobra.Command, args []string) {
	bot, err := discord.NewBot(db, config)
	if err != nil {
		slog.Error("error creating discord ci6ndex-bot", "guildId", guildId, "error", err)
		return
	}
